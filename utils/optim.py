...
def update_lr(max_lr, warmup_steps, total_steps, step, optimizer):
    """learning rate warm up and decay"""

    min_lr = 1e-3 * max_lr
    if step < warmup_steps:
        lr = (max_lr - min_lr) * step / warmup_steps + min_lr
    else:
        lr = min_lr + 0.5 * (max_lr - min_lr) * (1 + np.cos((step - warmup_steps) * np.pi / total_steps))
